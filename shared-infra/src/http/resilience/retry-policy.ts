 ...
 *    - Registers HTTP status codes (e.g. 400, 401, 403, 404, 422) and error codes (UNAUTHORIZED, FORBIDDEN, VALIDATION_ERROR, NOT_FOUND, ERR_CASCADE_LOOP) that should never be retried.
...
export class RetryPolicyRegistry {
  ...
  private readonly nonRetryableCodes = new Set<string>([
    "UNAUTHORIZED",
    "FORBIDDEN",
    "VALIDATION_ERROR",
    "NOT_FOUND",
  ...
  public isRetryable(err: any): boolean {
    ...
    if (err?.message && String(err.message).includes("Maximum cascade depth")) {
      return false;
    }
